...
While $\epsilon$ represents the same noise threshold between features in $\mathbf{F}$, $\tau$ is a threshold such that for any two representations, if $|\mathbf{r}_1 \cdot \mathbf{r}_2| > \tau$, they are deemed to have a feature in common and are labelled as neighbors.
We use the absolute dot product rather than cosine similarity because the latter normalizes out magnitude information: small-coefficient monosemantic representations have their shared-feature signal attenuated relative to $\epsilon$-interference noise after normalization, making them indistinguishable from non-sharing pairs.
With dot product, the shared feature contribution scales as $c_i c_j$ while interference scales as $\epsilon \cdot c_i c_j$, preserving the signal-to-noise ratio regardless of coefficient magnitude.

When the generative parameters are known, $\tau$ can be derived analytically.
The minimum dot product between representations sharing a feature is $c_{\min}^2$ (from the shared feature's coefficient product), while the maximum dot product between non-sharing representations is bounded by $k^2 c_{\max}^2 \epsilon$ (from $k \times k$ cross-feature interference terms).
Separation is guaranteed when $c_{\min}^2 > k^2 c_{\max}^2 \epsilon$, and the optimal $\tau$ lies between these bounds.
When the generative parameters are unknown, they can be treated as hyperparameters in a sweep.
...
Since the dot product threshold $\tau$ only requires the presence of a single shared feature to trigger a neighbor relationship, the resulting neighbor set $\mathbf{X}_{\text{pos}}$ is semantically heterogeneous.
